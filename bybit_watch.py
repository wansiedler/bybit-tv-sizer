...
async def tick(
    http: httpx.AsyncClient, before: dict[str, Position] | None, send, speak
) -> dict[str, Position]:
    ...
    open when the relay started are not news.
    """
...
async def poll(http: httpx.AsyncClient, send, speak) -> None:
    ...
    `send` posts to the bot, `speak` voices a line; both injected, which keeps
    this module free of a cycle with relay.py and lets tests drive it dry.
    """
    log.info("watching bybit positions every %ss", POLL)
    snapshot: dict[str, Position] | None = None
    while True:
        try:
            snapshot = await tick(http, snapshot, send, speak)
