...
def config(monkeypatch):
    ...
    monkeypatch.setattr(relay, "WATCH_USERS", ["aLexjjcrypt"])
...
class FakeWatchedEvent:
    ...
    def __init__(self, raw_text, sender=None, chat=None):
        ...
        self._sender = sender if sender is not None else SimpleNamespace(username="aLexjjcrypt")
        self._chat = chat if chat is not None else SimpleNamespace(title="LEXX Trading CLUB")
...
def test_run_relays_watched_user_messages(config, monkeypatch):
    ...
    assert http.posted[1] == "👤 @aLexjjcrypt · LEXX Trading CLUB:\nсетку ставим на OP"
