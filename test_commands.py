...
def test_status_text_reports_the_state(owner, monkeypatch):
    ...
    monkeypatch.setattr(commands, "WATCH_USERS", ["aLexjjcrypt", "second"])

    text = commands.status_text(stats, speaking=True)

    assert "up 1m30s" in text
    assert "source: lexx_dra_bot" in text
    assert "watching: @aLexjjcrypt, @second (all chats)" in text
...
def test_parse_watch_users_strips_prefixes_and_blanks():
    assert commands.parse_watch_users("aLexjjcrypt, @second ,, ") == ["aLexjjcrypt", "second"]
